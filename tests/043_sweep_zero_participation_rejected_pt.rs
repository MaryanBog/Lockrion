// ==============================
// tests/043_sweep_zero_participation_rejected_pt.rs
// ==============================
#![forbid(unsafe_code)]
...
async fn warp_until_ts(ctx: &mut ProgramTestContext, target_ts: i64) {
    loop {
        ...
        // test-clock model used in your other tests: "time" = slot/2
...
#[tokio::test]
async fn sweep_zero_participation_rejected_pt() {
    ...
    let airdrop_ix = system_instruction::transfer(&participant_pk, &platform_pk, 5_000_000_000);
    ...
    // mint & fund reserve so sweep has something to move
    mint_to(
        &mut ctx,
        &reward_mint.pubkey(),
    ...
    // -------- ensure escrow funded --------
    let escrow_bal = token_balance(&mut ctx, &reward_escrow.pubkey()).await;
    assert!(escrow_bal > 0, "reward escrow must be funded for sweep test");

    // -------- ZERO PARTICIPATION scenario: no deposits at all --------
    // move to sweep_start (claim window end)
    warp_until_ts(&mut ctx, sweep_start).await;

    // Attempt sweep should be rejected because total_weight_accum == 0 (zero participation)
    let sweep_ix = mk_ix(
        program_id,
    ...
    let err = send_tx_err(&mut ctx, vec![sweep_ix], &[]).await;

    // Expected: NoParticipation (this is the contract's sweep gate for zero participation)
    let expected = LockrionError::NoParticipation as u32;
