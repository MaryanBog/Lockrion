...
# LOCK_MINT (correct, stored in issuance)
LOCK_MINT_OK="$(spl-token create-token --decimals 0 --fee-payer "$FEE_PAYER" | awk '/Creating token/ {print $3}')"
DEPOSIT_ESCROW="$(spl-token create-account "$LOCK_MINT_OK" --owner "$ISSUANCE_PDA" --fee-payer "$FEE_PAYER" | awk '/Creating account/ {print $3}')"

# WRONG mint for participant ATA
LOCK_MINT_WRONG="$(spl-token create-token --decimals 0 --fee-payer "$FEE_PAYER" | awk '/Creating token/ {print $3}')"

PLATFORM_TREASURY="$ISSUER"

# init_issuance uses LOCK_MINT_OK
PROGRAM_ID="$PROGRAM_ID" START_TS="$START_TS" MATURITY_TS="$MATURITY_TS" RESERVE_TOTAL="$RESERVE_TOTAL" \
LOCK_MINT="$LOCK_MINT_OK" REWARD_MINT="$REWARD_MINT" DEPOSIT_ESCROW="$DEPOSIT_ESCROW" REWARD_ESCROW="$REWARD_ESCROW" \
...
# wait until start_ts (so mint error is the first gate)
while true; do
  ...
  if [ "$NOW_CHAIN" -ge "$START_TS" ]; then break; fi
...
# Participant ATA uses WRONG mint
PARTICIPANT_LOCK_ATA="$(spl-token create-account "$LOCK_MINT_WRONG" --owner "$ISSUER" --fee-payer "$FEE_PAYER" 2>&1 | awk '/Creating account/ {print $3}')"
spl-token mint "$LOCK_MINT_WRONG" "$DEPOSIT_AMOUNT" "$PARTICIPANT_LOCK_ATA" --fee-payer "$FEE_PAYER" >/dev/null
...
if [ $RC -eq 0 ]; then
  echo "$DEPOSIT_OUT"
  die "deposit succeeded but expected failure (InvalidMint)"
fi

# InvalidMint = 53 decimal => 0x35
echo "$DEPOSIT_OUT" | grep -Eqi "custom program error: 0x35" || {
  echo "$DEPOSIT_OUT"
  die "expected InvalidMint (0x35) but got different error"
}
...
echo "✅ TEST PASS: deposit invalid mint rejected"