// tests/025_substitute_reward_escrow_rejected_pt.rs
#![forbid(unsafe_code)]
...
#[tokio::test]
async fn substitute_reward_escrow_rejected_pt() {
    ...
    // escrow FAKE substitute (same mint/owner, but different pubkey)
    let fake_reward_escrow = Keypair::new();
    create_token_account(&mut ctx, &fake_reward_escrow, &reward_mint.pubkey(), &issuance_pda).await;
