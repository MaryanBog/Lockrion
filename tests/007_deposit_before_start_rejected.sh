...
# IMPORTANT: start in the future so deposit happens BEFORE start_ts
START_TS=$((CHAIN_NOW + 40))
...
# fund_reserve (ensure we hit time-gate, not ReserveNotFunded)
...
# Deposit attempt BEFORE start_ts (no wait)
...
if [ $RC -eq 0 ]; then
  echo "$DEPOSIT_OUT"
  die "deposit succeeded but expected failure (DepositWindowNotStarted)"
fi

# DepositWindowNotStarted = 20 decimal => 0x14
echo "$DEPOSIT_OUT" | grep -Eqi "custom program error: 0x14" || {
  echo "$DEPOSIT_OUT"
  die "expected DepositWindowNotStarted (0x14) but got different error"
}
...
echo "✅ TEST PASS: deposit before start rejected"