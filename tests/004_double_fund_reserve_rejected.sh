...
# Second funding (must fail)
set +e
AMOUNT="$RESERVE_TOTAL" ISSUER_REWARD_ATA="$ISSUER_REWARD_ATA" REWARD_ESCROW="$REWARD_ESCROW" \
PROGRAM_ID="$PROGRAM_ID" ISSUANCE_PDA="$ISSUANCE_PDA" node.exe tests/js/fund_reserve.js >/dev/null 2>&1
RC=$?
set -e

[ $RC -ne 0 ] || die "Second fund_reserve did not fail"

ESCROW_BAL="$(spl-token balance --address "$REWARD_ESCROW" | tr -d '\r')"
[ "$ESCROW_BAL" = "$RESERVE_TOTAL" ] || die "Escrow balance changed unexpectedly"

echo "✅ TEST PASS: double fund_reserve rejected"