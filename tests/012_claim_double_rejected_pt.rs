// tests/012_claim_double_rejected_pt.rs
#![forbid(unsafe_code)]
...
#[tokio::test]
async fn claim_double_rejected_program_test() {
    ...
    send_tx_ok(&mut ctx, vec![claim_ix], &[]).await;
    ...
    let code = send_tx_expect_custom(&mut ctx, vec![claim2_ix], &[]).await;
