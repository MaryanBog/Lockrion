...
# init_issuance (ВАЖНО: reserve НЕ funding)
...
# wait until start_ts so failure is strictly ReserveNotFunded (not "window not started")
while true; do
...
# Attempt deposit -> must FAIL with ReserveNotFunded = 11 (0x0b)
set +e
DEPOSIT_OUT="$(
...
if [ $RC -eq 0 ]; then
  echo "$DEPOSIT_OUT"
  die "deposit succeeded but expected failure (ReserveNotFunded)"
fi

echo "$DEPOSIT_OUT" | grep -Eqi "custom program error: 0x0?b" || {
  echo "$DEPOSIT_OUT"
  die "expected ReserveNotFunded (0xb) but got different error"
}
...
echo "✅ TEST PASS: deposit before funding rejected"