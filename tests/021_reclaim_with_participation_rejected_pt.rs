// tests/021_reclaim_with_participation_rejected_pt.rs
#![forbid(unsafe_code)]
...
#[tokio::test]
async fn reclaim_with_participation_rejected_pt() {
    ...
    // persist total_weight_accum > 0 (withdraw updates and stores issuance/user)
    ...
    // reclaim must be rejected because participation exists (total_weight_accum != 0)
    ...
    // NOTE: contract currently returns NoParticipation when total_weight_accum != 0 (naming inverted),
    // so expected custom error code is 42.
    send_expect_custom_err(
        &mut ctx,
        vec![reclaim_ix],
        LockrionError::NoParticipation as u32,
    )
    .await;
}