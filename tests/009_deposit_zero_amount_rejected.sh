...
# wait until start_ts so we don't hit DepositWindowNotStarted
while true; do
...
# participant lock ATA (even though amount=0; keep accounts valid)
...
# Attempt deposit(0) -> must FAIL with InvalidAmount = 23 => 0x17
set +e
DEPOSIT_OUT="$(
...
if [ $RC -eq 0 ]; then
  echo "$DEPOSIT_OUT"
  die "deposit(0) succeeded but expected failure (InvalidAmount)"
fi

echo "$DEPOSIT_OUT" | grep -Eqi "custom program error: 0x17" || {
  echo "$DEPOSIT_OUT"
  die "expected InvalidAmount (0x17) but got different error"
}
...
echo "✅ TEST PASS: deposit zero amount rejected"